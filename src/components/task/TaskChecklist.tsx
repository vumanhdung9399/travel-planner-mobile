...
export default function TaskChecklist({ trip }: { trip: Trip }) {
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.card}>
          <View style={styles.progressRow}>
            <View style={styles.progressIcon}><Ionicons name="checkmark-done-outline" size={22} color={COLORS.success} /></View>
            <View style={styles.progressBody}>
              <Text style={styles.progressTitle}>Tiến độ công việc</Text>
              <Text style={styles.subtitle}>Mọi thành viên cùng cập nhật</Text>
            </View>
          ...
          {readOnly || !trip.isLeader ? (
            <View style={styles.readOnlyBanner}>
              ...
              <Text style={styles.readOnlyText}>{readOnly ? 'Chuyến đi đã kết thúc · Chỉ xem' : 'Bạn chỉ có thể hoàn thành việc được giao cho mình'}</Text>
            </View>
          ) : null}
          {canManageTasks ? <View style={styles.quickAdd}>
            <TextInput value={title} onChangeText={setTitle} onSubmitEditing={createTask}
              placeholder="Thêm việc cần làm..." placeholderTextColor={COLORS.textLight}
              maxLength={180} returnKeyType="done" style={styles.input} />
        ...
        <View style={styles.listCard}>
          {loading ? <ActivityIndicator style={styles.empty} color={COLORS.primary} /> : visibleTasks.length === 0 ? (
            <View style={styles.empty}><Ionicons name="checkmark-done-circle-outline" size={38} color={COLORS.textLight} /><Text style={styles.emptyText}>Chưa có công việc trong bộ lọc này</Text></View>
            ...
            <View key={task.id} style={styles.row}>
              <TouchableOpacity onPress={() => toggleTaskStatus(task.id, !task.isCompleted)} disabled={!canToggleTask(task)}
                ...
                style={[styles.checkbox, task.isCompleted && styles.checkboxDone, !canToggleTask(task) && styles.readOnlyControl]}>
                {task.isCompleted && <Ionicons name="checkmark" size={16} color="#fff" />}
              </TouchableOpacity>
              <View style={styles.taskContent}>
                <Text numberOfLines={2} style={[styles.taskTitle, task.isCompleted && styles.taskDone]}>{task.title}</Text>
                <Text numberOfLines={1} style={styles.dueDate}>{task.assignee ? `Giao cho ${task.assignee.name}` : 'Chưa giao cho ai'}{task.dueDate ? `  •  Hạn ${new Date(task.dueDate).toLocaleDateString('vi-VN')}` : ''}</Text>
              </View>
              <TouchableOpacity onPress={() => setAssigningTaskId(task.id)} disabled={!canManageTasks} style={[styles.assignee, !canManageTasks && styles.readOnlyControl]}>
      ...
      <Modal visible={Boolean(assigningTaskId)} transparent animationType="slide" onRequestClose={() => setAssigningTaskId(null)}>
        <Pressable style={styles.overlay} onPress={() => setAssigningTaskId(null)}>
          <Pressable style={styles.sheet} onPress={(event) => event.stopPropagation()}>
            ...
            <Text style={styles.sheetTitle}>Giao việc cho</Text>
            <TouchableOpacity style={styles.memberRow} onPress={() => assignUserToTask(null)}>
              <View style={styles.unassigned}><Ionicons name="person-outline" size={18} color={COLORS.textSecondary} /></View>
              <Text style={styles.memberName}>Chưa giao cho ai</Text>
            </TouchableOpacity>
